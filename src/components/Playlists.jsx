...
const Playlists = () => {
  ...
  const { playlists, createPlaylist, allsongs, addSongtoPlaylist, currentTrackIndex ,setCurrentTrack , handlePlaySong , deletePlaylist} = useMusic()
  ...
  const filteredSongs = allsongs.filter((song) => {
    ...
    const isAlready = selectedPlaylist?.songs?.some((playlistSong) => playlistSong.id === song.id)
    return matches && !isAlready
              ...
              playlists.map((playlist, key) => (
                <div className='playlist-item' key={key}>
                        ...
                        selectedPlaylist?.id == playlist.id && showDropdown && (
                          <div className='song-dropdown'>
                            {filteredSongs.length == 0 ? (
                              ...
                              (filteredSongs.slice(0, 5).map((song, key) => <div key={key} className='dropdown-item'
                  ...
                  <div className="playlist-songs">
                    {playlist.songs.length === 0 ? (<p className='empty-playlist'>No songs in playlist</p>) :
                      playlist.songs.map((song, key) => (
                        <div className={`playlist-song ${currentTrackIndex === allsongs.findIndex((s) => s.id === song.id) ? "active" : ""}`} key={key}
